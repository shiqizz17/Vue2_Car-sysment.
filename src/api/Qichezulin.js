...
import QichezulinModel from '../models/QichezulinModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let zulinriqistart = req.query.zulinriqistart;
			let zulinriqiend = req.query.zulinriqiend;
			if (zulinriqistart && zulinriqiend) {
				let zulinriqibetween = [];
				zulinriqibetween.push(zulinriqistart);
				zulinriqibetween.push(zulinriqiend);
				where.zulinriqi = {
					[Op.between]: zulinriqibetween
				}
			}
			...
			var dingdanbianhao = req.query.dingdanbianhao
			if (dingdanbianhao) {
			...
			var chepaihao = req.query.chepaihao
			if (chepaihao) {

				if (chepaihao.indexOf('%') != -1) {
					where.chepaihao = {
						[Op.like]: chepaihao
					}
				} else {
			...
			var zulinriqi = req.query.zulinriqi
			if (zulinriqi) {

				if (zulinriqi.indexOf('%') != -1) {
					where.zulinriqi = {
						[Op.like]: zulinriqi
					}
				} else {
			...
			let result = await QichezulinModel.findAndCountAll({
				order: [[sort, order]],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await QichezulinModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await QichezulinModel.update(req.body, {
				where: {
				  id: req.body.id
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await QichezulinModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 - req.query.remindstart, "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM qichezulin WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
